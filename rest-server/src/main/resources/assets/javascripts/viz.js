...
//        ngrams=_.map(ngrams, function(d) {
        ...
        //add party name to each data
        ngrams = _.map(ngrams, function (d) {
            d.listDates = _.map(d.listDates, function (e) {
                    e.name = d.name;
                    return e;
                });
            return d;
...
//            .attr("style","visibility: visible;");
        ...
        var paths = parties.append("path").attr("d", function (d) {
            return path(d.listDates)
        })
        ...
        var tip = d3.tip()
            ...
            .html(function (d) {
                ...
                return "<span>Count: " + d.count + "</span><br/>" +
                    "<span>Month: " + date.getFullYear() + "-" + (date.getMonth() + 1) + "</span><br/>" +
                    "<span>Party: " + d.name + "</span>"
            })
        ...
        sbpath = d3.svg.line()
            .x(function (d) {
                return sbxscale(d.date);
            })
            .y(function (d) {
                return sbyscale(d.count);
            });
        ...
        sb.append("rect")
            .data([
                {"x": border - 2, "y": selectbarheight / 2 - 10}
            ])
            .attr("x", border - 2)
        ...
        sb.append("rect")
            .data([
                {"x": width - border - 2, "y": selectbarheight / 2 - 10}
            ])
...
function drawAxis() {
    ...
    var lines = [
        [border - 5, height - border, border - 5, border],
