...
rm -rf $3
echo "wystapienie_id\t;\tposel\t;\tstanowisko\t;\ttekst" > $3  #write CSV header

for i in $(seq $1 $2)
do
    echo "-------------------------"
    echo "Downloading with id=$i..."
    if wget http://sejmometr.pl/sejm_wystapienia/$i -q -O /tmp/wystapienie$i.html; then

        echo "Parsing HTML with id=$i to CSV file..."
        echo -n "$i\t;\t" >> $3 # append id
        if python wystapienia_parse.py < /tmp/wystapienie$i.html >> $3; then #append parsed data
